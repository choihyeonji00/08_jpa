...
public class NativeQueryRepository {
  ...
  public Menu nativeQueryByResultType(int menuCode) {
    ...
    * */
	  ...
	  String query 
		  = "SELECT" +
		    " menu_code, menu_name, menu_price, category_code, orderable_status" + // 하나 빼먹음 == 필드 5개 menuPrice 값 오류가 뜸 모든 컬럼은 무조건 조회가 되어야함.
	      " FROM tbl_menu" +
	      " WHERE menu_code = ?";

    // 조회 결과를 Menu 엔티티에 매핑시켜주는 코드
	  Query nativeQuery
    ...
    // 쿼리 수행 결과가 Object 이기 때문에 Menu로 다운 캐스팅
	  return (Menu) nativeQuery.getSingleResult(); // 메뉴로 다운캐스팅
