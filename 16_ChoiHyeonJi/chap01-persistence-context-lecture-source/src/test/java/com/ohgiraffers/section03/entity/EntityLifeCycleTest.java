...
class EntityLifeCycleTest {
  ...
  void testTransient(int menuCode) {
    ...
    // newMenu 비영속 : 동일한 값을 가지지만 직접 생성한 새 객체
    ...
    assertTrue(entityManager.contains(foundMenu)); //entityManager가 foundMenu를 포함하고 있는지 확인
  ...
  void testDetachEntity(int menuCode, int menuPrice) {
    ...
    // (현재) 준영속 상태 -> DB 테이블과 매핑이 X, 그냥 자바 객체의 값만 변경됨
    // (더티 체킹 X)
  ...
  void testDetachAndMerge(int menuCode, int menuPrice) {
    ...
    entityManager.flush(); // DB에 전달만 함 반영 XX(commit/rollback 가능)
  ...
  void testMergeUpdate(int menuCode, String menuName) {
    ...
    Menu foundMenu = entityManager.find(Menu.class, menuCode);
    entityManager.detach(foundMenu); // 준영속으로 변경
  ...
  @ParameterizedTest
  @ValueSource(ints = {1, 2, 3}) // 메뉴코드 1,2,3에 맞는
  void testClearPersistenceContext(int menuCode) {
    ...
    // foundMenu == 준영속
    // expectedMenu == 영속 서로 다른 객체니까 같지 않은 객체 assertNotSame
  ...
  @ParameterizedTest
  @ValueSource(ints = {1}) //1번 메뉴
  void testRemoveEntity(int menuCode) {
    // 해석하면서 진행하기
    //given
    EntityManager entityManager = EntityManagerGenerator.getInstance();
    EntityTransaction transaction = entityManager.getTransaction(); //entityManager를 통해 transaction 얻어옴
    Menu foundMenu = entityManager.find(Menu.class, menuCode); //menuCode똑같은애조회 영속상태에 foundMenu가 있음
    ...
    entityManager.remove(foundMenu); // 영속상태의 foundMenu 지움
    ...
    transaction.rollback(); //commit 하면 반영 rollback 하면 삭제된 것 처럼 보이지만 원래대로 되돌림.
  }
