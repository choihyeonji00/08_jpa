...
public class EntityManagerCRUD {
  ...
  public Menu findMenuByMenuCode(int menuCode) {
    ...
    return entityManager.find(Menu.class, menuCode); //tbl_menu 테이블 안에서 찾음 menuCode가 일치하는
  }
  ...
  public Long saveAndReturnAllCount(Menu newMenu) {
    ...
    EntityTransaction entityTransaction = entityManager.getTransaction(); // DB 관리 매니저
    ...
    // 반영하려면 Commit 수행
    entityManager.persist(newMenu); //persist 영속 안에 집어넣은거임
  ...
  public Long getCount(EntityManager entityManager) {
    ...
    return entityManager.createQuery(
        "select count(*) from Section02Menu", Long.class).getSingleResult(); //결과가 1행
  }
